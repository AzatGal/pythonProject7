...
from models.blocks.resnet_blocks import InputStem, StageB
...
class ResNetB(nn.Module):
    def __init__(self, cfg, nrof_classes):
        ...
        self.stages = nn.Sequential(
            StageB(nrof_blocks=3, in_channels=64, out_channels=64, stride=1),
            StageB(nrof_blocks=4, in_channels=128, out_channels=128, stride=2),
            StageB(nrof_blocks=6, in_channels=256, out_channels=256, stride=2),
            StageB(nrof_blocks=3, in_channels=512, out_channels=512, stride=2)
        )
